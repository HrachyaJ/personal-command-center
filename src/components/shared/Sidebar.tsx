...
function Sidebar() {
  ...
  return (
    <>
      ...
      {/* REMOVED: overflow-hidden from aside class list */}
      <aside
        className="hidden md:flex bg-card border-r border-border flex-col fixed top-0 left-0 h-screen z-40"
        style={{
          width: collapsed ? "5rem" : "16rem",
          transition: "width 300ms cubic-bezier(0.4, 0, 0.2, 1)",
        }}
        data-testid="sidebar"
      >
        {/* ADDED: Inner masking wrapper so internal content handles collapse animations securely */}
        <div className="w-full h-full flex flex-col overflow-hidden">
          {/* Header */}
          <div
            className="p-6 border-b border-border flex items-center overflow-hidden shrink-0"
            style={{ height: "73px" }}
          >
          ...
          {/* Nav — Handles long menu scrolls independently internally */}
          ...
          {/* Footer — Permanently stuck perfectly to the view frame edge */}
        ...
        {/* Toggle button — Placed cleanly outside the inner mask container */}
        <button
          onClick={() => setSidebarCollapsed(!collapsed)}
          className="absolute -right-3.5 top-[23px] bg-card border border-border text-foreground rounded-full p-1.5 shadow-md z-50 cursor-pointer hover:bg-accent hover:text-white"
          ...
          {navigation.map((item) => {
            ...
            return (
              <li key={item.name} className="flex-1">
                <button
                  onClick={() => navigate(item.href)}
                  className={`w-full h-full flex flex-col items-center justify-center gap-1 cursor-pointer transition-colors active:scale-[0.94] ${
                    isActive
                      ? "text-primary"
